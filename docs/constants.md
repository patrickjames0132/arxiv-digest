...
| `NBUCKETS` | Corpus citations hash-partition count | Baked into the ingested Parquet layout — a different value orphans every ingested release | 1024 | `src/atlas/integrations/semantic_scholar/corpus/ingest.py`, `.../corpus/source.py`, `src/ml_pipelines/live_pool_validation/collect.py` |
...
| Name | What it is | Why it's needed | Value | Referenced in |
|---|---|---|---|---|
| `PER_YEAR_CAP` | Max landmarks per publication year (STOP/SKIP bucket cap) | **Fitted** (cite_budget pipeline); makes "clutter" concrete — both sizing rules and the model label share it | 12 | `src/atlas/services/graph/budget.py`, `src/atlas/integrations/openalex/traversal.py`, `src/atlas/integrations/caps.py` (doc), `src/ml_pipelines/cite_budget/{collect,features,train}.py`, `src/ml_pipelines/live_pool_validation/collect.py` |
| `tau`, `max_span` | Latest-band start rule: density threshold + widest span | **Fitted** (latest_gap pipeline); live in the artifact, not code | 0.25 / 7 (in `model.joblib`) | `src/atlas/services/graph/bands.py`, `src/ml_pipelines/latest_gap/train.py` |
...
| `floor` (cite_budget) | Predicted budget's lower clamp | Stops the retired model guessing absurdly low (pipelines-only since v5.13.0) | 12 (in `model.joblib`) | `src/atlas/services/graph/budget.py`, `src/ml_pipelines/cite_budget/train.py` |
| `UNBOUNDED_LANDMARK_CAP` | The shared **payload guard** — every citation relation's flat ceiling | Never fitted, deliberately not config (settled v6.0.0): a mega seed can't page its citer list into one response | 500 | `src/atlas/integrations/caps.py` (home), `.../openalex/traversal.py`, `.../semantic_scholar/traversal.py`, `.../semantic_scholar/corpus/source.py`, `src/atlas/services/graph/budget.py`, `src/atlas/config.py` (doc), `src/atlas/integrations/__init__.py` (doc), `src/ml_pipelines/cite_budget/collect.py` |
...
| `REACHABLE_CITERS` | Most citers a live deep-page can return | Derived: `_MAX_OFFSET + _RANK_POOL` — the truncation boundary the validation study simulates | 9000 | `src/atlas/integrations/semantic_scholar/traversal.py`, `src/ml_pipelines/live_pool_validation/collect.py` |
| `_LATEST_YEARS` | Calendar years that are latest-only, never landmarks | Defines the landmark/latest boundary (`landmark_max_year`) — a relation *definition*, not a size | 2 | `src/atlas/integrations/openalex/traversal.py`, `src/ml_pipelines/latest_gap/train.py` |
